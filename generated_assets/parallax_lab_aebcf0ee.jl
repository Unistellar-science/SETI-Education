...
!!! note " "
	...
	*Source: [JustinWick](https://upload.wikimedia.org/wikipedia/commons/2/2e/Parallax_Example.png)*
...
!!! note " "
	![](https://upload.wikimedia.org/wikipedia/commons/e/e2/New_Horizons_Proxima_Centauri_Parallax_Animation.gif)
...
# ╔═╡ cfa533c7-f6a0-4df1-9846-6dfc5b76a57a
...
md"""
and store them into `img1` and `img2` for convenience:
"""
...
@bind observatory Select([OBSERVATORIES...])

# ╔═╡ e213ab0a-374f-4ad9-a884-f8c9c905a905
observatory
...
# ╔═╡ f1d4da0d-e980-4782-aa46-103c8eaf689f
img1.header
...
!!! note " "
	![](https://juliaimages.org/ImageTransformations.jl/stable/assets/warp_resize.png)

	*Source: [JuliaImages](https://juliaimages.org/ImageTransformations.jl/stable/#index_image_warping)*
...
!!! note " "
	![](https://www.dropbox.com/scl/fi/d3hk9vqo3cl73da737z42/parallax_diagram.svg?rlkey=t7p1vv2sjfk765r46k1100pej&e=2&st=v7bdh66b&raw=1)

	*Source: Modified from [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Parallax_Example.png)*
...
md"""
We can do a quick dimensions check with a units package like [DynamicQuantities.jl](https://ai.damtp.cam.ac.uk/dynamicquantities/stable/) to verify our results. Note that this will differ slightly from our estimated result because we are internally using a more accurate value for the AU when we make our conversion.
"""
...
!!! note " "
	![fig1](https://www.dropbox.com/scl/fi/dcky7peoqa3j7hr86zwju/orbit_sim.png?rlkey=7kpiw4423rxvy6i0zwgkqqg4e&st=ejfrdjzg&raw=1)
"""
...
!!! hint " "
	**Baseline**\
	Since ``\theta`` goes like ``\frac{b}{d}``, taking measurements over a larger baseline would yield a larger parallax shift that we could then measure to a more accurate degree.
	
	**Equatorial coordinates**\
	We could use a tool like [astrometry.net](https://nova.astrometry.net/) to [plate solve](https://en.wikipedia.org/wiki/Astrometric_solving) our images. This would allow us to directly calculate the ``\Delta\text{RA}`` and ``\Delta\text{DEC}`` of the asteroid to a high precision instead of relying on pixel coordinates.
	
	**Image transformation parameters**\
	We relied on an affine transformation to stack our images. While flexible, the increased number of parameters relative to simpler transformation schemes can introduce additional error. Perhaps only rotatation and translation would be enough, although this might require using telescopes with comparable plate scales and apertures (like an eVscope!).
	
	**Center identification**\
	Regardless of the stacking scheme and coordinate system chosen, our parallax measurement is only as good as our ability to identify the approximate center of the asteroid in each image. Fitting a point spread function ([PSF](https://en.wikipedia.org/wiki/Point_spread_function)) would be useful for accurately identifying the center coordinates of our asteroid instead of eyeballing it as we did in this lab.
"""

# ╔═╡ 6244900e-a58b-44e1-8c71-802e3fd29c10
md"""
...
function trace_hm(img; colorbar_x = 0)
    ...
    return heatmap(
        ...
        zmax = ZMAX,
        colorscale = :Cividis,
        colorbar = attr(x = colorbar_x, thickness = 10, title = "Counts", ypad = 0),
    )
end
...
function plot_pair(observatories)
    ...
    # Size the figure to match the aspect ratio of the images so that each
    # heatmap exactly fills its subplot, keeping the colorbars the same height
    # as the images. The 0.45 is each subplot's fraction of the plot width
    # (2 columns with the default 0.1 horizontal spacing, matching the
    # `colorbar_x` placement above)
    lo(d) = minimum(img -> first(dims(img, d)), (img1, img2))
    hi(d) = maximum(img -> last(dims(img, d)), (img1, img2))
    aspect = (hi(Y) - lo(Y) + 1) / (hi(X) - lo(X) + 1)
    margin = (l = 80, r = 80, t = 20, b = 80)
    width = 700
    height = round(Int, 0.45 * (width - margin.l - margin.r) * aspect + margin.t + margin.b)

    # Add a shared y-label
    relayout!(
        fig,
        Layout(yaxis_title = "Y (pixels)"),
        font_size = 10,
...
git-tree-sha1 = "988ff909479c5661b24f756432127198a7e465d0"
...
git-tree-sha1 = "5f5e34050f287973d9c626058d6a5683874b8959"
...
git-tree-sha1 = "3c57831a38899ebc8fed2af5267476959082a3c5"
