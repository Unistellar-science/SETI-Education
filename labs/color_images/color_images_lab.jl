...
md"""
What are some things that you notice? Try doing the same analysis on one of your own FITS images by:

1. dropping it into the same folder on your computer that this notebook is in.
1. Pasting the full name of the FITS file (including the extension) into the box below and clicking the `Load` button.
$(@bind fpath_img_sci confirm(TextField((65, 1)); label = "Load"))
"""
...
img_sci = if isempty(fpath_img_sci)
		img_astro_gray
	else
	load(fpath_img_sci)
end;
...
manifest_format = "2.0"
project_hash = "d300702f3219e1f0cd81f75ba96768682128590a"
